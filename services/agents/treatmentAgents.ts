...
import type { Patient, Message, TextMessage, GdmtChecklistMessage, GdmtChecklistItem, ContraindicationMessage, ContraindicationItem, DosageOptimizationMessage, DosageOptimizationItem, Report, AiPersonalizationSettings, PrescriptionMessage, SourceVerification } from '../../types';
...
export const runGuidelineAdherenceAgent = async (patient: Patient, query: string, ai: GoogleGenAI): Promise<GdmtChecklistMessage | TextMessage> => {
    if (!patient.medicalHistory.some(item => item.description.includes('Heart Failure (HFrEF)') || item.description.includes('Systolic Heart Failure'))) {
    ...
    const latestLab = patient.reports.filter(r => r.type === 'Lab').sort((a,b) => b.date.localeCompare(a.date))[0];
    ...
    // 1. ARNI / ACEi / ARB
    const hasArni = meds.some(m => m.includes('sacubitril'));
    const hasAcei = meds.some(m => m.includes('lisinopril') || m.includes('ramipril') || m.includes('enalapril') || m.includes('benazepril') || m.includes('quinapril'));
    const hasArb = meds.some(m => m.includes('valsartan') && !m.includes('sacubitril')) || meds.some(m => m.includes('losartan') || m.includes('candesartan') || m.includes('irbesartan') || m.includes('olmesartan') || m.includes('telmisartan'));
    ...
    if (hasArni) {
        ...
        const prescribedDrug = patient.currentStatus.medications.find(m => {
    ...
    // 2. Beta-Blocker
    const hasBb = meds.some(m => m.includes('carvedilol') || m.includes('metoprolol') || m.includes('bisoprolol'));
    if (hasBb) {
        const drug = meds.find(m => m.includes('carvedilol') || m.includes('metoprolol') || m.includes('bisoprolol'));
        items.push({ 
            status: 'prescribed', 
    ...
    // 3. MRA
    const hasMra = meds.some(m => m.includes('spironolactone') || m.includes('eplerenone'));
    if (hasMra) {
        items.push({ 
            status: 'prescribed', 
            drugClass: 'MRA', 
    ...
    const hasSglt2 = meds.some(m => sglt2s.some(s => m.includes(s)));
    
    if (hasSglt2) {
        const drugName = patient.currentStatus.medications.find(m => sglt2s.some(s => m.toLowerCase().includes(s))) || 'SGLT2i';
        items.push({ 
            status: 'prescribed', 
        ...
        const historyText = patient.medicalHistory.map(h => h.description.toLowerCase()).join(' ');
    ...
    const medsReport = patient.reports.find(r => r.type === 'Meds');
...
export const runDosageOptimizationAgent = async (patient: Patient, query: string, ai: GoogleGenAI, aiSettings: AiPersonalizationSettings): Promise<DosageOptimizationMessage | TextMessage> => {
    ...
    **Patient Data:**
    ...
    - **Diagnoses:** ${patient.medicalHistory.map(h => h.description).join(', ')}.
    - **Allergies:** ${patient.allergies.join(', ') || 'None'}.
