...
export const LiveAssistant: React.FC = () => {
  ...
  const { isLiveModeOpen, selectedPatient: patient, aiSettings } = state;
            ...
            if (message.toolCall) {
              ...
              for (const fc of message.toolCall.functionCalls) {
                ...
                if (fc.name === 'consult_specialist') {
                  ...
                  try {
                    const report = await apiManager.generateSpecialistReport(patient, specialty, aiSettings);
              ...
              biomarkers.slice().reverse().map((b) => (
                <div key={`${b.symptom}-${b.timestamp}`} className={`flex flex-col px-3 py-2 backdrop-blur-md rounded-lg border w-64 animate-slideInLeft shadow-lg ${b.type === 'visual' ? 'bg-purple-900/60 border-purple-500/30' : 'bg-blue-900/60 border-blue-500/30'}`}>
                  <div className="flex items-start justify-between w-full">
                    ...
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${b.severity === 'High' ? 'bg-red-500/40 text-red-100 border-red-500/50' :
